...
const ProjectDetailsClient = ({ project, slug }) => {
  ...
  const openLayoutGallery = (startIndex = 0) => {
    ...
    const galleryItems = layouts
      ...
      .map(layout => ({
        src: layout.image,
        opts: {
          caption: getText(layout.name, layout.nameArabic) || ''
        }
      }));
  ...
  useEffect(() => {
    ...
    const initFancybox = () => {
      if (typeof window !== 'undefined' && window.$ && window.$.fancybox) {
        ...
        const gallerySelector = `[data-fancybox="gallery-${locale}"]`;
        if (window.$(gallerySelector).length) {
          window.$(gallerySelector).each(function () {
            window.$(this).off('click.fb-start');
          });
        }

        // Initialize fancybox with locale-specific selector
        window.$(gallerySelector).fancybox({
          buttons: [
            "zoom",
            "slideShow",
            "fullScreen",
    ...
    return () => {
      clearTimeout(timer);
      // Cleanup: close any open fancybox instances
      if (typeof window !== 'undefined' && window.$ && window.$.fancybox) {
        const instance = window.$.fancybox.getInstance();
        if (instance) {
          instance.close(true);
  ...
  }, [foundProject, locale]); // Added locale to dependencies to reinitialize when language changes
  ...
  return (
    <>
      <Header1 fluid={"container-fluid"} />
      <Breadcrum content={getText(foundProject.name, foundProject.nameArabic)} pageTitle={t('project.projectDetails')} pagename={t('project.project')} />
      ...
      <div className="project-details-page pt-120 mb-120">
        <div className="container">
          <div className="row g-lg-4 gy-5 mb-120">
            <div className="col-lg-8">
              ...
              <div className="project-details-thumb mb-50">
                <img src={`${foundProject.mainImage}`} alt={getText(foundProject.name, foundProject.nameArabic)} />
              </div>

              <div className="details-content-wrapper" id="overview">
                <h2>{getText(foundProject.name, foundProject.nameArabic)}</h2>
                ...
                {foundProject.sections && foundProject.sections.map((section, sectionIndex) => (
                  <div key={sectionIndex} className="project-section-details mb-5">
                    <h3>{getText(section.title, section.titleArabic)}</h3>
                    <h4 className="project-subtitle mb-4">{getText(section.subtitle, section.subtitleArabic)}</h4>
                    <span className="line-break" />
                    <p>{getText(section.description, section.descriptionArabic)}</p>
                    <span className="line-break" />

                    {/* Section Image */}
                    {section.image && (
                      <div className="project-details-thumb mb-30">
                        <img src={`${section.image}`} alt={getText(section.title, section.titleArabic)} style={{ width: '100%' }} />
                      </div>
                    )}

                    {/* Section Categories */}
                    {(() => {
                      // Get the appropriate categories array based on language
                      const categoriesToDisplay = isRTL && Array.isArray(section.categoriesArabic) && section.categoriesArabic.length > 0
                        ? section.categoriesArabic
                        : (section.categories || []);
                      
                      return categoriesToDisplay.length > 0 ? (
                        <>
                          <h5>{t('project.categories')}</h5>
                          <ul className="mb-4">
                            {categoriesToDisplay.map((category, catIndex) => {
                              // Handle both string and object formats
                              const categoryName = typeof category === 'string' 
                                ? category 
                                : (isRTL && category.nameArabic ? category.nameArabic : (category.name || ''));
                              return (
                                <li key={catIndex}>
                ...
                <span className="line-break" />
                <h3>{t('project.keyFeatures')}</h3>
                <span className="line-break" />
                <p>{t('project.keyFeaturesDescription', { name: getText(foundProject.name, foundProject.nameArabic) })}</p>
                <span className="line-break" />
                  ...
                  <ul>
                    {foundProject.features.map((feature, index) => {
                      ...
                      const featureName = typeof feature === 'string' 
                        ? feature 
                        : (isRTL && feature.nameArabic ? feature.nameArabic : (feature.name || ''));
                      return (
                        <li key={index}>
                          <svg xmlns="http://www.w3.org/2000/svg" width={15} height={15} viewBox="0 0 15 15">
                            <path d="M0.376831 8.16821C-0.247095 8.54593 -0.0579659 9.49862 0.662688 9.60837C1.24211 9.69666 1.52052 10.3701 1.17304 10.8431C0.740845 11.4312 1.27942 12.2389 1.98713 12.0639C2.55609 11.9231 3.07065 12.4387 2.9302 13.0088C2.75556 13.718 3.56158 14.2577 4.14855 13.8246C4.62054 13.4764 5.29275 13.7554 5.38073 14.336C5.49024 15.0581 6.44099 15.2476 6.81798 14.6224C7.12107 14.1198 7.84864 14.1198 8.15171 14.6224C8.52867 15.2476 9.47943 15.0581 9.58896 14.336C9.67707 13.7554 10.3492 13.4764 10.8211 13.8246C11.4081 14.2577 12.2142 13.718 12.0395 13.0088C11.899 12.4387 12.4136 11.9231 12.9826 12.0639C13.6903 12.2389 14.2289 11.4312 13.7967 10.8431C13.4492 10.3701 13.7276 9.69653 14.307 9.60837C15.0276 9.49864 15.2168 8.54597 14.5929 8.16821C14.0912 7.86452 14.0912 7.13547 14.5929 6.83178C15.2168 6.45407 15.0277 5.50138 14.307 5.39162C13.7276 5.30334 13.4492 4.62989 13.7967 4.15695C14.2289 3.56879 13.6903 2.76112 12.9826 2.93613C12.4136 3.07687 11.8991 2.5613 12.0395 1.99115C12.2141 1.28199 11.4081 0.742345 10.8211 1.17541C10.3492 1.52356 9.67695 1.2446 9.58896 0.664029C9.47945 -0.0580599 8.5287 -0.247606 8.15171 0.377594C7.84863 0.880237 7.12106 0.880237 6.81798 0.377594C6.44103 -0.247596 5.49027 -0.0580833 5.38073 0.664029C5.29263 1.24462 4.62054 1.5236 4.14855 1.17541C3.56158 0.742345 2.75554 1.28201 2.9302 1.99115C3.07065 2.56126 2.55612 3.07686 1.98713 2.93613C1.2794 2.76113 0.740845 3.56879 1.17304 4.15695C1.52049 4.62989 1.24209 5.30346 0.662688 5.39162C-0.0579425 5.50136 -0.247105 6.45403 0.376831 6.83178C0.878459 7.13548 0.878459 7.86453 0.376831 8.16821Z" />
                  ...
                  <ul className="project-info">
                    <li>
                      ...
                      <div className="content">
                        ...
                        <h5>{getText(foundProject.client, foundProject.clientArabic)}</h5>
                      </div>
                    </li>
                    <li>
                      <div className="icon">
                      ...
                      <div className="content">
                        ...
                        <h5>{getText(foundProject.name, foundProject.nameArabic)}</h5>
                      </div>
                    </li>
                    <li>
                      <div className="icon">
                      ...
                      <div className="content">
                        ...
                        <h5>{getText(foundProject.location, foundProject.locationArabic)}</h5>
                      </div>
                    </li>
                    <li>
                      <div className="icon">
                      ...
                      <div className="content">
                        <span>{t('project.buildingType')}:</span>
                        <h5>{t('project.residentialTower')}</h5>
                      </div>
                    </li>
                    <li>
                      <div className="icon">
                    ...
                    <li>
                      ...
                      <div className="content">
                        <span>{t('project.specifications')}:</span>
                        <h5>{foundProject.floors} {t('project.floors')}, {foundProject.units} {t('project.units')}</h5>
                      </div>
                    </li>
                  </ul>
                </div>
                <div className="sidebar-banner">
                  ...
                  <div className="banner-content-wrap">
                    <div className="banner-content">
                      <h2>{t('project.readyToGetDreamHome')}</h2>
            ...
            <div className="col-lg-6">
              {foundProject.layouts && foundProject.layouts.length > 0 && (
                <div className="layouts-section" id="layouts">
                  <h3>{t('project.availableUnitLayouts')}</h3>
                  <span className="line-break" />
                  <p className="mb-4">{t('project.chooseLayoutDescription')}</p>
                      ...
                      >
                        <div className="d-flex gap-3 align-items-center">
                          {layout.image && (
                            <div className="layout-image flex-shrink-0" style={{ width: '90px', height: '90px' }}>
                              <div className="layout-image-wrapper">
                                <img src={`${layout.image}`} alt={getText(layout.name, layout.nameArabic)} />
                                <button
                                  type="button"
                                  className="layout-image-overlay"
                                  aria-label={`${t('project.viewLayout')} ${getText(layout.name, layout.nameArabic)}`}
                                  onClick={(event) => {
                          ...
                          <div className="flex-grow-1">
                            <h5 className="mb-2">{getText(layout.name, layout.nameArabic)}</h5>
                            <div className="layout-specs">
                              <div className="d-flex flex-wrap gap-2 mb-2">
                                <span><strong>{layout.area}</strong></span>
                                <span>{layout.bedrooms} {t('project.bed')}</span>
                                <span>{layout.bathrooms} {t('project.bath')}</span>
                              </div>
                            </div>
                                ...
                                <ul className="mb-0 ps-3">
                                  {layout.features.slice(0, 2).map((feature, featureIdx) => {
                                    ...
                                    const featureName = typeof feature === 'string' 
                                      ...
                                      : (isRTL && feature.nameArabic ? feature.nameArabic : (feature.name || ''));
                                    return (
                                      <li key={featureIdx} style={{ fontSize: '13px' }}>{featureName}</li>
                                    );
                                  })}
                                </ul>
                              </div>
            ...
            <div className="row mt-5">
              <div className="col-12">
                <div className="property-details-page" id="nearby">
                  <WhatsNearby
                    title={t('project.whatsNearby')}
                    description={t('project.whatsNearbyDescription')}
              ...
              <div className="col-12">
                <div className="property-details-page" id="documents">
                  <div className="property-details-content-wrap">
                    <PropertyAttachment
                      title={t('project.projectDocuments')}
                      description={t('project.projectDocumentsDescription')}
          ...
          {foundProject.gallery && foundProject.gallery.length > 0 && (
            <div className="row mt-5" key={`gallery-${locale}`}>
              <div className="col-12">
                <div className="project-gallery-section" id="gallery">
                  <h3 className="mb-4">{t('project.projectGallery')}</h3>
                  <p className="mb-5">{t('project.projectGalleryDescription')}</p>

                  <div className="row g-4">
                    {foundProject.gallery.map((item, index) => {
                      // Get category and title based on language
                      const categoryName = getText(item.category, item.categoryArabic);
                      const itemTitle = getText(item.title, item.titleArabic);
                      
                      return (
                        <div key={`${locale}-${index}`} className="col-lg-4 col-md-6">
                          <a
                            href={item.image}
                            data-fancybox={`gallery-${locale}`}
                            data-caption={`${categoryName} - ${itemTitle}`}
                          ...
                          >
                            ...
                            >
                              ...
                              <h5 className="text-white mb-0">{itemTitle}</h5>
                            </div>
                          </a>
                        </div>
                      );
